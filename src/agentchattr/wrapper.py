...
Usage:
    ...
    uv run agentchattr agent gemini
    uv run agentchattr agent kimi
    uv run agentchattr agent qwen

Linux: injects prompts via tmux bracketed paste (wrapper_unix.py)
...
def _build_provider_launch(
    ...
    *,
    ...
    Linux these are prefixed onto the tmux command via ``env VAR=val``
...
def main(config: dict, args, extra: list[str]):
    import urllib.error
    import urllib.request
    ...
    try:
        ...
        print("  Start the server first: uv run agentchattr serve (with matching config/ports).")
        sys.exit(1)
    ...
    from agentchattr.wrapper_unix import get_activity_checker, run_agent
