...
def main(config: dict, *, allow_network: bool = False):
    logging.basicConfig(
        level=logging.INFO,
    ...
    from agentchattr.app import app, configure, set_event_loop
    configure(config, session_token=session_token)

    # Share stores with the MCP bridge
    from agentchattr.app import store, rules, summaries, jobs, room_settings, registry, router as app_router, agents as app_agents, session_engine
